...
data class TreeNode<out K, out V>(val nodes: List<Pair<K, Tree<K, V>>>) :
  Tree<K, V>() {
...
fun <K, V, R> Tree<K, V>.mapKey(mapper: (K) -> R): Tree<R, V> = when (this) {
  is TreeLeaf -> this
  is TreeNode -> nodes
    .map { (key, value) -> mapper(key) to value.mapKey(mapper) }
    .let(::TreeNode)
}

fun <K, V> Tree<K, V>.asSequence(path: TreePath<K> = emptyPath)
  : Sequence<Pair<TreePath<K>, V>> =
  when (this) {
    is TreeLeaf -> sequenceOf(path to value)
    is TreeNode -> nodes.asSequence()
      .flatMap { (key, value) -> value.asSequence(path + key) }
  }

operator fun <K, V> Tree<K, V>.get(key: K): Tree<K, V>? = when (this) {
