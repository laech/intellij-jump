...
private fun getFoldRegionByStartOffset(editor: Editor): NavigableMap<Int, FoldRegion> =
...
private fun getAreaStartOffset(editor: Editor, area: Rectangle): Int =
    editor.logicalPositionToOffset(editor.xyToLogicalPosition(Point(area.x, area.y)))

private fun getAreaEndOffset(editor: Editor, area: Rectangle): Int =
    editor.logicalPositionToOffset(
        editor.xyToLogicalPosition(Point(area.x + area.width, area.y + area.height)))

fun getSingleStrokeEditorEscapeKeys(actionManager: ActionManager): Set<KeyStroke> =
    (actionManager.getActionOrStub(ACTION_EDITOR_ESCAPE)?.shortcutSet?.shortcuts ?: emptyArray())
        .asSequence()
